...
def get_first_level_jobs(config, jenkins):
    url = "{}/api/json?tree=jobs[name]".format(jenkins)
    headers = {'Content-Type': 'application/json'}
    r = requests.get(url, auth=(config['Username'], config['API_Token']), headers=headers)
    return r